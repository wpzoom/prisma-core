 ...
 * @since 1.0.0
 */
class Prisma_Core_Plugin_Utilities {
	...
	/**
	 * Main Prisma Core Plugin Utilities Instance.
	 *
	 * @since 1.0.0
	 * @return Prisma_Core_Plugin_Utilities
	 */
	public static function instance() {

		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Prisma_Core_Plugin_Utilities ) ) {
	...
	public function activate_plugin( $plugin ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error(
				...
				esc_html__( 'Current user can\'t activate plugins', 'prisma-core' )
			);
		}
		...
		if ( empty( $plugin ) ) {
			return new WP_Error(
				'plugin_activation_failed',
				esc_html__( 'Missing plugin data.', 'prisma-core' )
			);
		}
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Plugin %s is not installed.', 'prisma-core' ),
					$plugin
				)
			);
		}
		...
		do_action( 'prisma_core_plugin_activated_' . $plugin );
	}
	...
	public function deactivate_plugin( $plugin ) {
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					// translators: %s is plugin name.
					esc_html__( 'Plugin %s is not active.', 'prisma-core' ),
					$plugin
				)
			);
		}
		...
		do_action( 'prisma_core_plugin_deactivated_' . $plugin );
	}
	...
	public function get_recommended_plugins() {
		...
		return apply_filters( 'prisma_core_recommended_plugins', $plugins );
	}
 ...
 *
 * Example: <?php $prisma_core_plugin_utilities = prisma_core_plugin_utilities(); ?>
 *
 * @since 1.0.0
 * @return object
 */
function prisma_core_plugin_utilities() {
	return Prisma_Core_Plugin_Utilities::instance();
}
